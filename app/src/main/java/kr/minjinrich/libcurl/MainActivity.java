...
import kr.minjinrich.libcurl.data.CurlWorkerURL;
...
public class MainActivity extends AppCompatActivity
{
	...
	class CurlProc implements Runnable
	{
		...
		public void run()
		{
			p_current_hour = timeUtil.getCurrentHour();
			current_cnt = 0;
				...
				while( !Thread.currentThread().isInterrupted() )
				{
					...
					if( 0 < (n_current_hour - p_current_hour) )
					{
						...
						for( int i = 0; i < max_count; i++ )
						{
							...
							int rand_time = (rand.nextInt(1500 - 600) + 600) * 100;
							boolean	res = Curl.curl( pathCACert, url, 5, null );

							runOnUiThread( () -> setTextLastStatus( Boolean.toString( res ) ) );

							Log.d("CurlProcRunnable", "Random Time [" + rand_time + "] / Url [" + url + "] / Result [" + res + "]");
							if( (max_count - 1) == i )
							{
